...
import (
    ...
    // "github.com/aws/aws-sdk-go-v2/aws"  // Fixed the comment issue
...
func (c *ConfigChecks) Run(ctx context.Context) ([]CheckResult, error) {
    ...
    if err != nil || len(recorders.ConfigurationRecorders) == 0 {
        results = append(results, CheckResult{
            ...
            Remediation: "Enable AWS Config immediately",
            RemediationDetail: "Go to AWS Config Console → Get Started → Follow setup wizard",
...
func (c *GuardDutyChecks) Run(ctx context.Context) ([]CheckResult, error) {
    ...
    if err != nil || len(detectors.DetectorIds) == 0 {
        results = append(results, CheckResult{
            ...
            RemediationDetail: "Go to GuardDuty Console → Get Started → Enable GuardDuty",
