...
func (c *AADChecks) CheckGuestAccess(ctx context.Context) []CheckResult {
    ...
    for pager.More() {
        ...
        for _, assignment := range page.Value {
            ...
            // PrincipalType not available in current SDK version
            // Check for guest patterns in PrincipalID instead
    ...
    // Service Principal check - rough estimate based on total assignments
    // In practice would need Graph API to properly distinguish service principals from users
    servicePrincipalCount := 0
    
    // Rough estimate: if we have many assignments, some are likely service principals
    if totalAssignments > 50 {
        servicePrincipalCount = totalAssignments / 3 // Rough estimate: 1/3 might be service principals
    }
    ...
    if servicePrincipalCount > 20 {
        results = append(results, CheckResult{
            ...
            Status:            "FAIL",
            Severity:          "MEDIUM",
            Evidence:          fmt.Sprintf("Estimated %d service principals with role assignments - review for unused", servicePrincipalCount),
